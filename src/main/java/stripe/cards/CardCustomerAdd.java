 ...
 *
 */
@WebServlet(name = "CardCustomerAdd", urlPatterns = {"/CardCustomerAdd"})
public class CardCustomerAdd extends HttpServlet {
	...
	Map<String, Object> entityParamsToken = new HashMap<>();
	Map<String, Object> entityParamsCard = new HashMap<>();
...
// Builder
			...
			entityParamsCard.clear();
			entityParamsToken.clear();
			cardParams.clear();

			cardParams.put("number", uxPayload.getNumber());
			cardParams.put("exp_year", uxPayload.getYear());
			cardParams.put("exp_month", uxPayload.getMonth());
			cardParams.put("currency", uxPayload.getCurrency());

			// Optional
			if (uxPayload.getName().length() > 0) {
				cardParams.put("name", uxPayload.getName());
			}
			if (uxPayload.getCvc().length() > 0) {
				cardParams.put("cvc", uxPayload.getCvc());
			}

			// Direct Attach
			//cardParams.put("account", "");
			// entityParams.put("customer", uxPayload.getCustomerId());
			//cardParams.put("recipient", "");
			// Ready To Create
			// Create Token
			// Pass Token As Source
			if (uxPayload.getAccountId().length() > 0) {
				entityParamsToken.put("card", cardParams);
				token = Token.create(entityParamsToken, auth);

				entityParamsCard.put("source", token.getId());
				entity.getSources().create(entityParamsCard, auth);
//				entity = PaymentMethod.create(entityParams, auth);
			} else {
				entityParamsToken.put("card", cardParams);
				token = Token.create(entityParamsToken);

				entityParamsCard.put("source", token.getId());
				entity.getSources().create(entityParamsCard);
//				entity = PaymentMethod.create(entityParams);
			}

		} catch (StripeException ex) {
			System.out.println("ERR: " + ex.getMessage());
			System.out.println("INF: " + gson.toJson(uxPayload));
			Logger.getLogger(CardCustomerAdd.class.getName()).log(Level.SEVERE, null, ex);
			return;
		}
		System.out.println("INF: " + gson.toJson(uxPayload));
