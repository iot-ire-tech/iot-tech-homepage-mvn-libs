 ...
 *
 */
@WebServlet(name = "CardCustomerDelete", urlPatterns = {"/CardCustomerDelete"})
public class CardCustomerDelete extends HttpServlet {
...
// Builder
			...
			System.out.println("INF: Card Id (" + uxPayload.getCardId() + ")");
			if (uxPayload.getAccountId().length() > 0) {
			...
			if (uxPayload.getAccountId().length() > 0) {
				card = (Card) entity.getSources().retrieve(uxPayload.getCardId(), auth);
			} else {
				card = (Card) entity.getSources().retrieve(uxPayload.getCardId());
			}
			card.delete().setDeleted(Boolean.TRUE);

		} catch (StripeException ex) {
			System.out.println("ERR: " + ex.getMessage());
			System.out.println("INF: " + gson.toJson(uxPayload));
			Logger.getLogger(CardCustomerDelete.class.getName()).log(Level.SEVERE, null, ex);
			return;
		}
		System.out.println("INF: " + gson.toJson(uxPayload));
