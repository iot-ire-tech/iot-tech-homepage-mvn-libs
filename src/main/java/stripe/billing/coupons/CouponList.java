 ...
 *
 */
@WebServlet(name = "CouponsList", urlPatterns = {"/CouponsList"})
public class CouponList extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        System.out.println("INF: Processing Connect Couponse");
        params.put("limit", 100);

        try {
            if (uxPayload.getAccountId().length() > 0) {
                entity = Coupon.list(params, auth);
            } else {
                entity = Coupon.list(params);
            }
