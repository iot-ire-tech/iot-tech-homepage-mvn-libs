...
/**
 */
@WebServlet(name = "SubscriptionCancel", urlPatterns = {"/SubscriptionCancel"})
public class SubscriptionCancel extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// Cancel NOW
		subParams.clear();
		if (uxPayload.isProrate()) {
			subParams.put("prorate", uxPayload.isProrate());
		}
		if (uxPayload.isInvoiceNow()) {
			subParams.put("invoice_now", uxPayload.isInvoiceNow());
		}

		System.out.println("INF: Processing Subscriptions (" + uxPayload.getSubscriptionId() + ")");
		try {
			if (uxPayload.getAccountId().length() > 0) {
				entity = Subscription.retrieve(uxPayload.getSubscriptionId(), auth);
				entity.cancel(subParams, auth);
			} else {
				entity = Subscription.retrieve(uxPayload.getSubscriptionId());
				entity.cancel(subParams);
			}
		} catch (StripeException ex) {
			response.getOutputStream().print(gson.toJson(entity));
			Logger.getLogger(SubscriptionCancel.class.getName()).log(Level.SEVERE, null, ex);
			if (ex.getMessage().contains("No such subscription")) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST, "No such subscription");
