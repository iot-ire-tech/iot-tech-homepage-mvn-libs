...
public class ProductDelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            System.out.println("INF: Processed Product Delete Starting");
            System.out.println("INF: Account ID: " + uxPayload.getAccountId());
            System.out.println("INF: Product ID: " + uxPayload.getProductId());

            if (uxPayload.getAccountId().length() > 0) {
                oldEntity = Product.retrieve(uxPayload.getProductId(), auth);
            } else {
                oldEntity = Product.retrieve(uxPayload.getProductId());
            }

            if (uxPayload.getAccountId().length() > 0) {
                entity = oldEntity.delete(auth);
            } else {
                entity = oldEntity.delete();
            }
            ...
            if (msg.contains("This product cannot be deleted because it has one or more user-created prices")) {
        ...
        System.out.println("INF: Processed Product Delete Completed");
    }
