 ...
 *
 */
@WebServlet(name = "ProductsGetMode", urlPatterns = {"/ProductsGetMode"})
public class ProductsGetMode extends HttpServlet {
	...
	ArrayList<Product> entitysFull = new ArrayList<>();
	Product entity;
	Product last;
	Product previousN_1;
	RequestOptions auth;
...
//			entityParams.put("limit", uxPayload.getLimit());
			...
			while (true) {
				...
				for (Product next : entitys.autoPagingIterable()) {
					rt++;
					// Build up list in array.
					metadata = next.getMetadata();

					for (Map.Entry<String, String> entry : metadata.entrySet()) {
						System.out.println("Key = " + entry.getKey() + ", Value = " + entry.getValue());
						if (entry.getValue().equalsIgnoreCase(uxPayload.getMode())) {
							entitysFull.add(next);
						}
					}
					last = next;
				}
				...
				// Update To Next Item
				if (null == previousN_1) {
					System.out.println("ERR: Breaking, this account has no meta!!!");
					throw new IllegalArgumentException();
				}

				System.out.println("INF: Previous Id " + previousN_1.getId() + " Versus Last Id (" + last.getId() + ")");

				if (previousN_1.getId().contentEquals(last.getId())) {
					System.out.println("INF: List Exhausted....");
					break;
				} else {
			...
			Logger.getLogger(ProductsGetMode.class.getName()).log(Level.SEVERE, null, ex);
			return;
		} catch (IllegalArgumentException npe) {
			response.getOutputStream().print(gson.toJson(new ArrayList<String>()));
			response.getOutputStream().flush();
			return;
		...
		response.getOutputStream().print(gson.toJson(entitysFull));
		response.getOutputStream().flush();
