 ...
 *
 */
@WebServlet(name = "ProductGetBookings", urlPatterns = {"/ProductGetBookings"})
public class ProductsGetBookings extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			for (Map.Entry< String, String> entry : metadata.entrySet()) {
				...
				if (entry.getKey().contains("bookings")) {
			...
			response.getOutputStream().flush();
			Logger.getLogger(ProductsGetBookings.class.getName()).log(Level.SEVERE, null, ex);
		}

		// Return Product Listings.
		System.out.println("INF: " + gson.toJson(uxPayload));
		response.getOutputStream().print("{\"open\":false}");
		response.getOutputStream().flush();
