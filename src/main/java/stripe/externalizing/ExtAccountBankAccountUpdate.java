 ...
 *
 */
@WebServlet(name = "ExtBankAccountUpdate", urlPatterns = {"/ExtBankAccountUpdate"})
public class ExtAccountBankAccountUpdate extends HttpServlet {
...
// Builder
			System.out.println("INF: Processed Bank Starting");
			if (uxPayload.getAccountId().length() > 0) {
				entity = Account.retrieve(uxPayload.getAccountId(), auth);
			} else {
				entity = Account.retrieve();
			}

			if (uxPayload.getAccountId().length() > 0) {
				exAccount = entity.getExternalAccounts().retrieve(uxPayload.getTokenId(), auth);
			} else {
				exAccount = entity.getExternalAccounts().retrieve(uxPayload.getTokenId());
			}

			//
			// Lets update
			//
			entityParams.put("default_for_currency", uxPayload.isDefaultForCurrency());
			if (uxPayload.getAccountHolderName().length() > 0) {
				entityParams.put("account_holder_name", uxPayload.getAccountHolderName());
			}
			if (uxPayload.getAccountHolderType().length() > 0) {
				entityParams.put("account_holder_name", uxPayload.getAccountHolderType());
			}
			...
			//
			if (uxPayload.getAccountId().length() > 0) {
				exAccount.update(entityParams, auth);
			} else {
				exAccount.update(entityParams);
			}

		} catch (StripeException ex) {
			System.out.println("ERR: " + ex.getMessage());
			System.out.println("INF: " + gson.toJson(uxPayload));
			Logger.getLogger(ExtAccountBankAccountUpdate.class.getName()).log(Level.SEVERE, null, ex);
			return;
		}
		System.out.println("INF: " + gson.toJson(uxPayload));
		...
		System.out.println("INF: Processed Bank Completed");
	}
