 ...
 *
 */
@WebServlet(name = "ExtBankAccountDelete", urlPatterns = {"/ExtBankAccountDelete"})
public class ExtAccountBankAccountDelete extends HttpServlet {
	...
	Token token;
	RequestOptions auth;
	List<ExternalAccount> entitys = new ArrayList<>();
...
// Builder
			...
			List<ExternalAccount> extAccounts = entity.getExternalAccounts().getData();
			for (ExternalAccount a : extAccounts) {
				if (uxPayload.getTokenId().contentEquals(a.getId())) {
					a.delete();
				}
			}
//			extAccount.setDeleted(Boolean.TRUE);

		} catch (StripeException ex) {
			System.out.println("ERR: " + ex.getMessage());
			System.out.println("INF: " + gson.toJson(uxPayload));
			Logger.getLogger(ExtAccountBankAccountDelete.class.getName()).log(Level.SEVERE, null, ex);
			return;
		}
