 ...
 *
 */
@WebServlet(name = "PlatformPayouts", urlPatterns = {"/PlatformPayouts"})
public class PayoutsPlatform extends HttpServlet {
	...
	String destinationIdBank = "ba_1D1gduFOjjfpNUIxUpuNC227"; // TSB Account /
	String destinationId = destinationIdBank;
	Payout entity;
	RequestOptions auth;
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			System.out.println("INF: Processed Payout(X) Starting");
// https://stripe.com/docs/api/payouts/create
			entityParams.clear();
			entityParams.put("description", "Test: deposit entering platform account ");
			entityParams.put("amount", 321);
			entityParams.put("currency", "eur");
			entityParams.put("destination", destinationId);
			// Money to be taken from this so not bank_account

			entity = Payout.create(entityParams, auth);
			responseLoad = entity.toJson();
			response.getOutputStream().print(responseLoad);

			System.out.println("INF: Processed Payout(X) Completed");
		} catch (StripeException ex) {
			response.getOutputStream().print("{ "
				+ "\"errorMsg\": \"" + ex.getMessage() + "\", "
				+ "\"status\": \"failed\" "
				+ "}");
			Logger.getLogger(PayoutsPlatform.class.getName()).log(Level.SEVERE, null, ex);
		}
