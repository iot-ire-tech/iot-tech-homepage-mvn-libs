...
import stripe.billing.subscriptions.products.*;
...
/**
 */
@WebServlet(name = "ProductConnectGoods", urlPatterns = {"/ProductConnectGoods"})
public class ProductConnectGoods extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			uxPayload = gson.fromJson(sb.toString(), UXPayloadProduct.class);
			...
			System.out.println("INF: Processed Product Creation(platform) Starting");

			productParams.clear();
			productParams.put("name", "stickers");
			productParams.put("type", "good");
			productParams.put("description", "high gloss finish");

			prodAttribs.add("small");
			productParams.put("attributes", prodAttribs);
...
// https://stripe.com/docs/api/products/create
			...
			System.out.println("INF: Processed Product Creation(platform) Completed");

		} catch (StripeException ex) {
			Logger.getLogger(ProductConnectGoods.class.getName()).log(Level.SEVERE, null, ex);
	...
	public void init() throws ServletException {
		...
		gson = new Gson();
		sb = new StringBuilder();
		uxPayload = new UXPayloadProduct();
