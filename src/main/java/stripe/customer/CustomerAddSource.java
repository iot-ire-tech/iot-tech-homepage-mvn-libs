 ...
 *
 */
@WebServlet(name = "CustomerSourceAdd", urlPatterns = {"/CustomerSourceAdd"})
public class CustomerAddSource extends HttpServlet {
...
// Builder
			System.out.println("INF: Processed Customer Source Starting");
			if (uxPayload.getAccountId().length() > 0) {
			...
			if (uxPayload.getAccountId().length() > 0) {
//				BankAccount bankAccount = (BankAccount) entity.getSources().create(entityParams, auth);
				entity.getSources().create(entityParams, auth);
			} else {
				entity.getSources().create(entityParams);
			}

		} catch (StripeException ex) {
			System.out.println("ERR: " + ex.getMessage());
			System.out.println("INF: " + gson.toJson(uxPayload));
			response.getOutputStream().print("{'id':'" + ex.getMessage() + "'}");
			response.getOutputStream().flush();
			Logger.getLogger(CustomerAddSource.class.getName()).log(Level.SEVERE, null, ex);
			return;
		}
		System.out.println("INF: " + gson.toJson(uxPayload));
		...
		response.getOutputStream().flush();
		System.out.println("INF: Processed Customer Source Completed");
	}
