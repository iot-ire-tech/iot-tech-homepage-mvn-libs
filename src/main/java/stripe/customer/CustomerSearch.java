 ...
 */
@WebServlet(name = "CustomerFind", urlPatterns = {"/CustomerFind"})
public class CustomerSearch extends HttpServlet {
    ...
    CustomerCollection entitys = null;
    RequestOptions auth;
...
// Account Magic
            ...
            // search  token is Email! - SpanId Token!!!
            findBySpanId:
            {
                entityParams.clear();
                entityParams.put("limit", uxPayload.getLimit());
                CustomerCollection customerCollection = Customer.list(entityParams, auth);
                Iterable<Customer> x = customerCollection.autoPagingIterable();
                for (Customer customer : x) {
                    System.out.println("INF: Incoming customer (" + customer.getEmail() + ")");
                    System.out.println("INF: Spain Id  (" + uxPayload.getPerson().getEmail() + ")\n");

                    if (customer.getEmail().contentEquals(uxPayload.getPerson().getEmail())) {
                        System.out.println("INF: Match found ");
                        entity = customer;
                        break;
                    } else {
                        entity = new Customer();
