...
//					nextC.delete();
					System.out.println("INF: (" + rt++ + ")New Email " + next.getEmail() + " Versus Token (" + uxPayload.getPerson().getEmail() + ")");
					if (next.getEmail() == null || next.getPhone() == null) {
						System.out.println("INF: Null Email/Phone Found, Skipping To Next Customer");
						continue;
					} else if (next.getEmail().contentEquals(uxPayload.getPerson().getEmail())) {
						// Factor#1
						System.out.println("INF: Factor#1 Found(" + next.getId() + ")!!!");
						System.out.println("INF: New Email " + next.getEmail() + " Versus Token (" + uxPayload.getPerson().getEmail() + ")");
						System.out.println("INF: New Phone " + next.getPhone() + " Versus Token (" + uxPayload.getPerson().getPhone() + ")");
						if (next.getPhone().contentEquals(uxPayload.getPerson().getPhone())) {
							// Factor#22
							System.out.println("INF: Factor#2 Found!!!");
							response.getOutputStream().print(next.toJson());
							response.getOutputStream().flush();
							System.out.println("INF: Hit Count " + rt + ")");
				...
				if (null != last) {
					System.out.println("INF: Last Id " + previousN_1.getId() + " Versus Next Id (" + last.getId() + ")");
					if (previousN_1.getId().contentEquals(last.getId())) {
						System.out.println("INF: List Exhausted....Customer Not Authenticated");
						response.getOutputStream().print("{\"ERR\":\"Customer List Exhausted - Customer Not Authenticated\"}");
						response.getOutputStream().flush();
						return;
					}
				}
				entityParams.put("starting_after", last.getId());
				System.out.println("INF: RT " + rt + ")");
			}
// No Match Found
			...
			response.getOutputStream().flush();
			Logger.getLogger(CustomerGetAll.class.getName()).log(Level.SEVERE, null, ex);
		}
