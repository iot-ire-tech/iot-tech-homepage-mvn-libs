 ...
 *
 */
@WebServlet(name = "RefundAdd", urlPatterns = {"/RefundAdd"})
public class Refunds extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		entityParams.put("charge", uxPayload.getChargeId());
		if (uxPayload.getTransaction().getAmount() > 0) {
			entityParams.put("amount", uxPayload.getTransaction().getAmount());
		}
		entityParams.put("reason", uxPayload.getReason());
		entityParams.put("refund_application_fee", uxPayload.isReturnAppFee());
