...
var uxExternalAccountManagment = function () {
	this.counter = 0;
	this.html = "";
	...
	this.addHeader = function (actionMsg, accountType) {
		...
		if (this.accountType === "bank_account") {
			...
			this.html += "<tr><th>Brand</th><th>Country</th><th>Curreny</th><th>Funding</th><th>Last Four Digits</th><th>Expiry Date[yyyy/mm]</th><th>" + this.actionMsg + "</th></tr>"
		}
		return this;
	};
	...
	this.addBody = function (rowData, accountType, nameTag) {
		...
		this.rowData.forEach(function (data) {
			...
			if (data.object === "bank_account" && this.accountType === "bank_account") {
				...
				this.accountCounter++
				this.detected = true
				this.html += "<tr><td>" + data.brand + " (" + data.available_payout_methods + ")</td><td>" + data.country + "</td><td>" + data.currency + "(" + data.default_for_currency + ")</td><td>" + data.funding + "</td><td>" + data.last4 + "</td><td>" + data.exp_year + "/" + data.exp_month + "</td><td> <input id=" + data.id + " type=radio name=" + this.nameTag + "> </td></tr>"
			}
		}.bind(this))
		return this;
	};
...
function uxAccountBankUpdate(cardSourcesRsp) {
	...
	var extAccountListing = new uxExternalAccountManagment()
