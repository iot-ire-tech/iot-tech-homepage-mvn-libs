...
f.kConsoleLog(c.message);break;case "load":c.success?(a.version=c.version,a._defaultRuntimeSettings=c._defaultRuntimeSettings,f.kConsoleLog&&f.kConsoleLog("load dbr worker success"),b()):h(c.exception);URL.revokeObjectURL(n);break;case "task":try{d[c.id](c.body),d[c.id]=void 0}catch(u){throw d[c.id]=void 0,u;}break;default:f.kConsoleLog&&f.kConsoleLog(c)}};var e=document.createElement("a");e.href=a._resourcesPath||"./";var p=void 0;a._wasmjsResourcePath&&(p=document.createElement("a"),p.href=a._wasmjsResourcePath||
"./",p=p.href);var q=void 0;a._wasmResourcePath&&(q=document.createElement("a"),q.href=a._wasmResourcePath||"./",q=q.href);c.postMessage({type:"loadWorker",_resourcesPath:e.href,_wasmjsResourcePath:p,_wasmResourcePath:q,origin:location.origin})}).catch(function(b){h(b)})})};a.prototype._createWorkerInstance=function(){var b=this;return new Promise(function(c,e){var h=Math.random();d[h]=function(d){return d.success?d.instanceID?(b._instanceID=d.instanceID,c()):e(a.BarcodeReaderException(a.EnumErrorCode.DBR_NULL_REFERENCE,
...
if(b.crossOrigin&&"anonymous"!=b.crossOrigin)throw"cors";var n,m,g;if(2>=c.length){var p=n=0;var r=m=b.videoWidth;var l=g=b.videoHeight}else 4>=c.length?(p=n=0,r=b.videoWidth,l=b.videoHeight,m=c[1],g=c[2]):(p=c[1],n=c[2],r=c[3],l=c[4],m=c[5],g=c[6]);d="string"==typeof c[c.length-1]?c[c.length-1]:"";k=document.createElement("canvas");k.width=m;k.height=g;var t=k.getContext("2d");a._bAddOriVideoCanvasToResult&&(e=document.createElement("canvas"),e.width=b.videoWidth,e.height=b.videoHeight,e.getContext("2d").drawImage(b,
0,0));t.drawImage(e||b,p,n,r,l,0,0,m,g);h()})).then(function(){return a._decodeCanvas(k,d)}).then(function(b){if(a._bAddOriVideoCanvasToResult)for(var d=$jscomp.makeIterator(b),c=d.next();!c.done;c=d.next())c.value.oriVideoCanvas=e;if(a._bAddSearchRegionCanvasToResult)for(d=$jscomp.makeIterator(b),c=d.next();!c.done;c=d.next())c.value.searchRegionCanvas=k;return b})};a.prototype._decodeBase64=a.prototype.decodeBase64String=function(b,a){var d=this;if(!("string"==typeof b||"object"==typeof b&&b instanceof
String))return Promise.reject("'decodeBase64(base64Str, templateName)': Type of 'base64Str' should be 'String'.");"data:image/"==b.substring(0,11)&&(b=b.substring(b.indexOf(",")+1));a=a||"";return new Promise(function(h){if(c._bNodejs)h(d._decodeArrayBuffer(Buffer.from(b,"base64"),a));else{for(var e=atob(b),k=e.length,f=new Uint8Array(k);k--;)f[k]=e.charCodeAt(k);c._bDom?h(d._decodeBlob(new Blob([f]),a)):h(d._decodeUint8Array(f,a))}})};a.prototype._decodeUrl=function(b,a){var d=this;return new Promise(function(h,
e){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))throw TypeError("'_decodeUrl(url, templateName)': Type of 'url' should be 'String'.");a=a||"";if(c._bNodejs)(b.startsWith("https")?require("https"):require("http")).get(b,function(b){if(200==b.statusCode){var c=[];b.on("data",function(b){c.push(b)}).on("end",function(){h(d._decodeArrayBuffer(Buffer.concat(c),a))})}else e("http get fail, statusCode: "+b.statusCode)});else{var k=new XMLHttpRequest;k.open("GET",b,!0);k.responseType=
c._bDom?"blob":"arraybuffer";k.send();k.onloadend=function(){c._bDom?h(d._decodeBlob(this.response,a)):h(d._decodeArrayBuffer(this.response,a))};k.onerror=function(){e(k.error)}}})};a.prototype._decodeFilePath=function(b,a){var d=this;return new Promise(function(c,e){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))throw TypeError("'_decodeFilePath(path, templateName)': Type of 'path' should be 'String'.");a=a||"";require("fs").readFile(b,function(b,h){b?e(b):c(d._decodeArrayBuffer(h,
...
6,1,4,0,65,1,11]);try{var l=d.put(new WebAssembly.Module(e.buffer),"testStoreModule");l.onsuccess=function(){d.put(!0,"bSupportStoreModuleInDb").onsuccess=function(){b("set bSupportStoreModuleInDb = true success")}};l.onerror=function(){c("Failed to store [testStoreModule] in wasm cache, bSupportStoreModuleInDb == false: "+(l.error.message||l.error))}}catch(A){c("Failed to store [testStoreModule] in wasm cache, bSupportStoreModuleInDb == false: "+(A.message||A))}};d.onsuccess=function(){d.result?
...
((new Date).getTime()-l));var t=(new Date).getTime();return WebAssembly.instantiate(c,f).then(function(a){e.kConsoleLog&&kConsoleLog("build instance from module timecost: "+((new Date).getTime()-t));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve({module:c,instance:a})})}e.kConsoleLog&&kConsoleLog("get a wasm binary from db, timecost:"+((new Date).getTime()-l));var w=(new Date).getTime();return WebAssembly.instantiate(c,f).then(function(c){e.kConsoleLog&&
kConsoleLog("build instance from binary timecost: "+((new Date).getTime()-w));if(b){var f=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,c.module).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-f));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve(c)})}e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve(c)})},function(l){e.kConsoleLog&&
kConsoleLog(l.message||l);e.kConsoleLog&&kConsoleLog("downloading...");var t=(new Date).getTime();return b?WebAssembly.instantiateStreaming(fetch(c),f).then(function(b){d._onWasmDownloaded&&d._onWasmDownloaded();e.kConsoleLog&&kConsoleLog("download with build timecost: "+((new Date).getTime()-t));var c=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,b.module).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-c));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+
((new Date).getTime()-g));return Promise.resolve(b)},function(a){e.kConsoleLog&&kConsoleLog(a.message||a);return Promise.reject("Can't store wasm in db.")})}):fetch(c).then(function(a){return a.arrayBuffer()}).then(function(b){e.kConsoleLog&&kConsoleLog("download timecost: "+((new Date).getTime()-t));d._onWasmDownloaded&&d._onWasmDownloaded();var c=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,b).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-c));return Promise.resolve(b)},
...
c.onFrameRead;this.duplicateForgetTime=c.duplicateForgetTime;void 0==this.duplicateForgetTime&&(this.duplicateForgetTime=3E3);this.onNewCodeRead=c.onNewCodeRead;this._isOpen=!1};g.BarcodeReader.Scanner.prototype.isOpen=function(){return this._isOpen};g.BarcodeReader.Scanner.prototype.open=function(){var c=this;if(c._isOpen)return Promise.reject("The scanner is already open.");c._isOpen=!0;var f=c.htmlElement,a,e,d,m,b;(function(){for(var c=[],g=$jscomp.makeIterator(f.children),h=g.next();!h.done;h=
g.next())c.push(h.value);for(g=0;g<c.length;++g){var k=$jscomp.makeIterator(c[g].children);for(h=k.next();!h.done;h=k.next())c.push(h.value)}c=$jscomp.makeIterator(c);for(h=c.next();!h.done;h=c.next())h=h.value,h.classList.contains("dbrScanner-video")?a=h:h.classList.contains("dbrScanner-sel-camera")?e=h:h.classList.contains("dbrScanner-sel-resolution")?(d=h,d.options.length||(d.innerHTML='<option class="dbrScanner-opt-gotResolution" value="got"></option><option data-width="3840" data-height="2160">ask 3840 x 2160</option><option data-width="2560" data-height="1440">ask 2560 x 1440</option><option data-width="1920" data-height="1080">ask 1920 x 1080</option><option data-width="1600" data-height="1200">ask 1600 x 1200</option><option data-width="1280" data-height="720">ask 1280 x 720</option><option data-width="800" data-height="600">ask 800 x 600</option><option data-width="640" data-height="480">ask 640 x 480</option><option data-width="640" data-height="360">ask 640 x 360</option>',
m=d.options[0])):h.classList.contains("dbrScanner-opt-gotResolution")?m=h:h.classList.contains("dbrScanner-btn-close")&&(b=h)})();var h=void 0,p=c._updateDevice=function(){return navigator.mediaDevices.enumerateDevices().then(function(b){var c=[],d;if(e){var f=e.value;e.innerHTML=""}for(var g=0;g<b.length;++g){var l=b[g];if("videoinput"==l.kind){var k={};k.deviceId=l.deviceId;k.label=l.label||"camera "+g;c.push(k)}}b=$jscomp.makeIterator(a.srcObject.getTracks());for(g=b.next();!g.done;g=b.next()){g=
g.value;if(m)break;if("video"==g.kind)for(l=$jscomp.makeIterator(c),k=l.next();!k.done;k=l.next())if(k=k.value,g.label==k.label){var m=k;h=k.deviceId;break}}if(e){b=$jscomp.makeIterator(c);for(k=b.next();!k.done;k=b.next())g=k.value,l=document.createElement("option"),l.value=g.deviceId,l.innerText=g.label,e.appendChild(l),f==g.deviceId&&(d=l);f=e.childNodes;if(!d&&m&&f.length)for(f=$jscomp.makeIterator(f),b=f.next();!b.done;b=f.next())if(b=b.value,m.label==b.innerText){d=b;break}d&&(e.value=d.value)}return Promise.resolve({current:m,
all:c})})};c._pausevideo=function(){a.pause()};var n=function(){a.srcObject&&(self.kConsoleLog&&self.kConsoleLog("======stop video========"),a.srcObject.getTracks().forEach(function(a){a.stop()}))},q=c._playvideo=function(b,e,f){return new Promise(function(g,k){n();self.kConsoleLog&&self.kConsoleLog("======before video========");var l=c.videoSettings=c.videoSettings||{video:{facingMode:{ideal:"environment"}}};/Safari/.test(navigator.userAgent)&&/iPhone/.test(navigator.userAgent)&&!/Chrome/.test(navigator.userAgent)?
1280<=e?l.video.width=1280:640<=e?l.video.width=640:320<=e&&(l.video.width=320):(e&&(l.video.width={ideal:e}),f&&(l.video.height={ideal:f}));b?(l.video.facingMode=void 0,l.video.deviceId={exact:b}):l.video.deviceId=l.video.deviceId||{ideal:h};var t=!1,w=function(){self.kConsoleLog&&self.kConsoleLog("======try getUserMedia========");self.kConsoleLog&&self.kConsoleLog("ask "+JSON.stringify(l.video.width)+"x"+JSON.stringify(l.video.height));navigator.mediaDevices.getUserMedia(l).then(function(b){self.kConsoleLog&&
self.kConsoleLog("======get video========");return new Promise(function(c,e){a.srcObject=b;a.onloadedmetadata=function(){self.kConsoleLog&&self.kConsoleLog("======play video========");a.play().then(function(){self.kConsoleLog&&self.kConsoleLog("======played video========");var b="got "+a.videoWidth+"x"+a.videoHeight;d&&m&&(m.setAttribute("data-width",a.videoWidth),m.setAttribute("data-height",a.videoHeight),d.value="got",m.innerText=b);self.kConsoleLog&&self.kConsoleLog(b);c();g({width:a.videoWidth,
height:a.videoHeight})},function(a){e(a)})};a.onerror=function(){e()}})}).catch(function(a){self.kConsoleLog&&self.kConsoleLog(a);!t&&l.video?(t=!0,l.video.deviceId=void 0,l.video.width=void 0,l.video.height=void 0,w()):k(a)})};w()})};c.arrDiffCodeInfo=[];var k,r=function(a,b){if(a instanceof Array){var c=[];a=$jscomp.makeIterator(a);for(var d=a.next();!d.done;d=a.next())c.push(r(d.value,b));return c}c=JSON.parse(JSON.stringify(a,function(a,b){if("oriVideoCanvas"!=a&&"searchRegionCanvas"!=a)return b}));
b||(c.oriVideoCanvas=a.oriVideoCanvas,c.searchRegionCanvas=a.searchRegionCanvas);return c},v=function(){if(c._isOpen){if(a.paused)return self.kConsoleLog&&self.kConsoleLog("Video is paused. Ask in 1s."),setTimeout(v,1E3);self.kConsoleLog&&self.kConsoleLog("======= once read =======");var b=(new Date).getTime();Promise.all([JSON.stringify(k.getRuntimeSettings())!=JSON.stringify(c.runtimeSettings)?k.updateRuntimeSettings(c.runtimeSettings):Promise.resolve(),function(){var b=c.searchRegion;if(b&&(b.sx||
...
!0),time:d,count:1});if(c.onFrameRead)c.onFrameRead(r(a));a=$jscomp.makeIterator(e);for(k=a.next();!k.done;k=a.next())if(d=k.value,c.onNewCodeRead)c.onNewCodeRead(d.BarcodeText,r(d));setTimeout(v,c.intervalTime)}).catch(function(a){self.kConsoleLog&&self.kConsoleLog(a);setTimeout(v,c.intervalTime);throw a;})}else k&&(k.deleteInstance(),k=void 0)},y=function(){q(e.value).then(function(){c._isOpen||n()}).catch(function(a){alert("Play video failed: "+(a.message||a))})};e&&e.addEventListener("change",
y);var x=function(){if(d&&-1!=d.selectedIndex){var a=d.options[d.selectedIndex];var b=a.getAttribute("data-width");a=a.getAttribute("data-height")}q(void 0,b,a).then(function(){c._isOpen||n()}).catch(function(a){alert("Play video failed: "+(a.message||a))})};d&&d.addEventListener("change",x);var u=!!f.parentNode,z=c._closeWindow=function(){n();c._isOpen=!1;e&&e.removeEventListener("change",y);d&&d.removeEventListener("change",x);c._closeWindow=void 0;b&&b.removeEventListener("click",z);var a=f.parentNode;
a&&!u&&a.removeChild(f)};b&&b.addEventListener("click",z);u||document.body.appendChild(f);return Promise.all([g.BarcodeReader.loadWasm(),q()]).then(function(a){if(!c._isOpen)return n(),Promise.resolve();k=new g.BarcodeReader;var b=k.getRuntimeSettings();b.mAntiDamageLevel=3;b.mDeblurLevel=0;for(var d in c.runtimeSettings)void 0!=b[d]&&(b[d]=c.runtimeSettings[d]);c.runtimeSettings=b;return Promise.all([a[1],k.updateRuntimeSettings(b)])}).then(function(a){v();return Promise.all([a[0],p()])}).then(function(a){var b=
a[0];a=a[1];return Promise.resolve({width:b.width,height:b.height,current:a.current,all:a.all})})};g.BarcodeReader.Scanner.prototype.play=function(c,f,a){return this._isOpen?this._playvideo(c,f,a):Promise.reject("The scanner is not open.")};g.BarcodeReader.Scanner.prototype.pause=function(){this._isOpen&&this._pausevideo()};g.BarcodeReader.Scanner.prototype.close=function(){this._isOpen&&this._closeWindow()};g.BarcodeReader.Scanner.prototype.updateDevice=function(){return this._isOpen?this._updateDevice():
navigator.mediaDevices.enumerateDevices().then(function(c){for(var f=[],a=0;a<c.length;++a){var e=c[a];if("videoinput"==e.kind){var d={};d.deviceId=e.deviceId;d.label=e.label||"camera "+a;f.push(d)}}return Promise.resolve({current:void 0,all:f})})};return g.BarcodeReader});
