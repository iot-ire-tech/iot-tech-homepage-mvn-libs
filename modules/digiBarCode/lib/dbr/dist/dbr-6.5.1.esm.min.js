...
void 0;a._wasmjsResourcePath=void 0;a._wasmResourcePath=void 0;a._isShowRelDecodeTimeInResults=!1;a._bCustomWorker=void 0;a._bWithio=void 0;a._onWasmDownloaded=void 0;a.createInstance=function(b){return a.loadWasm().then(function(){return Promise.resolve(new a(b))})};var d={};a._loadWorker=function(){return new Promise(function(b,g){var c=a._workerName||"dbr-"+a._jsVersion+".min.js";if(a._workerResourcePath||a._resourcesPath){var m=a._workerResourcePath||a._resourcesPath;"/"!=m.charAt(m.length-1)&&
(m+="/");c=m+c}fetch(c).then(function(b){return b.blob()}).then(function(c){var m=URL.createObjectURL(c);c=a._dbrWorker=new Worker(m);c.onerror=function(b){b="worker error, you should host the page in web service: "+b.message;f.kConsoleLog&&f.kConsoleError(b);g(b)};c.onmessage=function(c){c=c.data;switch(c.type){case "log":f.kConsoleLog&&f.kConsoleLog(c.message);break;case "load":c.success?(a.version=c.version,a._defaultRuntimeSettings=c._defaultRuntimeSettings,f.kConsoleLog&&f.kConsoleLog("load dbr worker success"),
b()):g(c.exception);URL.revokeObjectURL(m);break;case "task":try{d[c.id](c.body),d[c.id]=void 0}catch(t){throw d[c.id]=void 0,t;}break;default:f.kConsoleLog&&f.kConsoleLog(c)}};var e=document.createElement("a");e.href=a._resourcesPath||"./";var p=void 0;a._wasmjsResourcePath&&(p=document.createElement("a"),p.href=a._wasmjsResourcePath||"./",p=p.href);var q=void 0;a._wasmResourcePath&&(q=document.createElement("a"),q.href=a._wasmResourcePath||"./",q=q.href);c.postMessage({type:"loadWorker",_resourcesPath:e.href,
_wasmjsResourcePath:p,_wasmResourcePath:q,origin:location.origin})}).catch(function(b){g(b)})})};a.prototype._createWorkerInstance=function(){var b=this;return new Promise(function(c,e){var g=Math.random();d[g]=function(d){return d.success?d.instanceID?(b._instanceID=d.instanceID,c()):e(a.BarcodeReaderException(a.EnumErrorCode.DBR_NULL_REFERENCE,"Can't create BarcodeReader instance.")):e(d.exception)};a._dbrWorker.postMessage({type:"createInstance",id:g,licenseKey:b._licenseKey})})};a.prototype.deleteInstance=
function(){var b=this;if(this._instance)this._instance.delete();else return new Promise(function(c,e){var g=Math.random();d[g]=function(b){return b.success?c():e(b.exception)};a._dbrWorker.postMessage({type:"deleteInstance",id:g,instanceID:b._instanceID})})};a.prototype._decodeBlob=function(b,a){var d=this;if(!(f.Blob&&b instanceof Blob))return Promise.reject("'_decodeBlob(blob, templateName)': Type of 'blob' should be 'Blob'.");a=a||"";return c._bDom?(new Promise(function(a,d){var c=URL.createObjectURL(b),
g=new Image;g.dbrObjUrl=c;g.src=c;g.onload=function(){a(g)};g.onerror=function(){d(TypeError("'_decodeBlob(blob, templateName)': Can't convert the blob to image."))}})).then(function(b){return d._decodeImage(b)}):(new Promise(function(a){var d=new FileReader;d.onload=function(){a(d.result)};d.readAsArrayBuffer(b)})).then(function(b){return d._decodeArrayBuffer(b,a)})};a.prototype._decodeArrayBuffer=function(b,a){if(!(f.arrayBuffer&&b instanceof ArrayBuffer||f.Buffer&&b instanceof Buffer))return Promise.reject("'_decodeBlob(arrayBuffer, templateName)': Type of 'arrayBuffer' should be 'ArrayBuffer' or 'Buffer'.");
a=a||"";return c._bDom?this._decodeBlob(new Blob(b),a):this._decodeUint8Array(new Uint8Array(b))};a.prototype._decodeUint8Array=function(b,d){var g=this;if(!(f.Uint8Array&&b instanceof Uint8Array||f.Uint8ClampedArray&&b instanceof Uint8ClampedArray))return Promise.reject("'_decodeBlob(uint8Array, templateName)': Type of 'uint8Array' should be 'Uint8Array'.");d=d||"";return c._bDom?g._decodeBlob(new Blob(b),d):new Promise(function(c){if(a._isShowRelDecodeTimeInResults){var m=(new Date).getTime(),h=
g._instance.DecodeFileInMemory(b,d);m=(new Date).getTime()-m;h=k(h);h._decodeTime=m;return c(h)}return c(k(g._instance.DecodeFileInMemory(b,d)))})};a.prototype._decodeImage=function(b,a){var d=this;return(new Promise(function(d){if(!(f.HTMLImageElement&&b instanceof HTMLImageElement))throw TypeError("'_decodeImage(image, templateName)': Type of 'image' should be 'HTMLImageElement'.");if(b.crossOrigin&&"anonymous"!=b.crossOrigin)throw"cors";a=a||"";d()})).then(function(){var c=document.createElement("canvas");
...
0,b.width,b.height).data;a(c)})).then(function(g){return c._decodeRawImageUint8Array(g,b.width,b.height,4*b.width,a.EnumImagePixelFormat.IPF_ARGB_8888,d)})};a.prototype._decodeVideo=a.prototype.decodeVideo=function(b){var a=this,d,c=arguments,e,h;return(new Promise(function(g){if(!(f.HTMLVideoElement&&b instanceof HTMLVideoElement))throw TypeError("'_decodeVideo(video [ [, sx, sy, sWidth, sHeight], dWidth, dHeight] [, templateName] )': Type of 'video' should be 'HTMLVideoElement'.");if(b.crossOrigin&&
"anonymous"!=b.crossOrigin)throw"cors";var m,k,p;if(2>=c.length){var r=m=0;var q=k=b.videoWidth;var l=p=b.videoHeight}else 4>=c.length?(r=m=0,q=b.videoWidth,l=b.videoHeight,k=c[1],p=c[2]):(r=c[1],m=c[2],q=c[3],l=c[4],k=c[5],p=c[6]);d="string"==typeof c[c.length-1]?c[c.length-1]:"";h=document.createElement("canvas");h.width=k;h.height=p;var n=h.getContext("2d");a._bAddOriVideoCanvasToResult&&(e=document.createElement("canvas"),e.width=b.videoWidth,e.height=b.videoHeight,e.getContext("2d").drawImage(b,
0,0));n.drawImage(e||b,r,m,q,l,0,0,k,p);g()})).then(function(){return a._decodeCanvas(h,d)}).then(function(b){if(a._bAddOriVideoCanvasToResult)for(var d=$jscomp.makeIterator(b),c=d.next();!c.done;c=d.next())c.value.oriVideoCanvas=e;if(a._bAddSearchRegionCanvasToResult)for(d=$jscomp.makeIterator(b),c=d.next();!c.done;c=d.next())c.value.searchRegionCanvas=h;return b})};a.prototype._decodeBase64=a.prototype.decodeBase64String=function(b,a){var d=this;if(!("string"==typeof b||"object"==typeof b&&b instanceof
String))return Promise.reject("'decodeBase64(base64Str, templateName)': Type of 'base64Str' should be 'String'.");"data:image/"==b.substring(0,11)&&(b=b.substring(b.indexOf(",")+1));a=a||"";return new Promise(function(g){if(c._bNodejs)g(d._decodeArrayBuffer(Buffer.from(b,"base64"),a));else{for(var e=atob(b),h=e.length,f=new Uint8Array(h);h--;)f[h]=e.charCodeAt(h);c._bDom?g(d._decodeBlob(new Blob([f]),a)):g(d._decodeUint8Array(f,a))}})};a.prototype._decodeUrl=function(b,a){var d=this;return new Promise(function(g,
e){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))throw TypeError("'_decodeUrl(url, templateName)': Type of 'url' should be 'String'.");a=a||"";if(c._bNodejs)(b.startsWith("https")?require("https"):require("http")).get(b,function(b){if(200==b.statusCode){var c=[];b.on("data",function(b){c.push(b)}).on("end",function(){g(d._decodeArrayBuffer(Buffer.concat(c),a))})}else e("http get fail, statusCode: "+b.statusCode)});else{var h=new XMLHttpRequest;h.open("GET",b,!0);h.responseType=
c._bDom?"blob":"arraybuffer";h.send();h.onloadend=function(){c._bDom?g(d._decodeBlob(this.response,a)):g(d._decodeArrayBuffer(this.response,a))};h.onerror=function(){e(h.error)}}})};a.prototype._decodeFilePath=function(b,a){var d=this;return new Promise(function(c,e){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))throw TypeError("'_decodeFilePath(path, templateName)': Type of 'path' should be 'String'.");a=a||"";require("fs").readFile(b,function(b,g){b?e(b):c(d._decodeArrayBuffer(g,
a))})})};a.prototype._decodeRawImageBlob=function(b,a,d,c,e,h){var g=this;return(new Promise(function(a,d){if(!(f.Blob&&b instanceof Blob))throw TypeError("'_decodeRawImageBlob(buffer, width, height, stride, enumImagePixelFormat, templateName)': Type of 'buffer' should be 'Blob'.");h=h||"";var c=new FileReader;c.readAsArrayBuffer(b);c.onload=function(){a(c.result)};c.onerror=function(){d(c.error)}})).then(function(b){return g._decodeRawImageUint8Array(new Uint8Array(b),a,d,c,e,h)})};a.prototype._decodeRawImageArrayBuffer=
function(b,a,d,c,e,h){var g=this;return(new Promise(function(a){if(!(f.ArrayBuffer&&b instanceof ArrayBuffer))throw TypeError("'_decodeRawImageArrayBuffer(buffer, width, height, stride, enumImagePixelFormat, templateName)': Type of 'buffer' should be 'ArrayBuffer'.");h=h||"";a()})).then(function(){return g._decodeRawImageUint8Array(new Uint8Array(b),a,d,c,e,h)})};a.prototype._decodeRawImageUint8Array=function(b,c,e,m,q,h){var g=this;return(new Promise(function(a){if(!(f.Uint8Array&&b instanceof Uint8Array||
f.Uint8ClampedArray&&b instanceof Uint8ClampedArray))throw TypeError("'_decodeRawImageUint8Array(buffer, width, height, stride, enumImagePixelFormat, templateName)': Type of 'buffer' should be 'Uint8Array'.");h=h||"";g._localizationResultArray=void 0;a()})).then(function(){if(g._instance){if(a._isShowRelDecodeTimeInResults){var f=(new Date).getTime(),p=g._instance.DecodeBuffer(b,c,e,m,q,h);f=(new Date).getTime()-f;p=k(p);p._decodeTime=f;return Promise.resolve(p)}return Promise.resolve(k(g._instance.DecodeBuffer(b,
c,e,m,q,h)))}var r;p=g._instanceID?Promise.resolve():g._createWorkerInstance();return p.then(function(){return null===g._runtimeSettings?new Promise(function(b,c){var l=Math.random();d[l]=function(a){return a.success?(g._runtimeSettings=void 0,b()):c(a.exception)};a._dbrWorker.postMessage({type:"resetRuntimeSettings",id:l,instanceID:g._instanceID})}):Promise.resolve()}).then(function(){return new Promise(function(f,k){var l=Math.random();d[l]=function(b){return b.success?f(b.results):k(b.exception)};
a._dbrWorker.postMessage({type:"decodeRawImageUint8Array",id:l,instanceID:g._instanceID,body:{buffer:b,width:c,height:e,stride:m,enumImagePixelFormat:q,templateName:h},_isShowRelDecodeTimeInResults:a._isShowRelDecodeTimeInResults})})}).then(function(b){r=b;return new Promise(function(b,c){var e=Math.random();d[e]=function(a){return a.success?b(a.results):c(a.exception)};a._dbrWorker.postMessage({type:"getAllLocalizationResults",id:e,instanceID:g._instanceID})})}).then(function(b){g._localizationResultArray=
...
a.prototype._decodeRawImage=a.prototype.decodeBuffer=function(b,a,c,d,e,h){return f.Blob&&b instanceof Blob?this._decodeRawImageBlob(b,a,c,d,e,h):f.ArrayBuffer&&b instanceof ArrayBuffer?this._decodeRawImageArrayBuffer(b,a,c,d,e,h):f.Uint8Array&&b instanceof Uint8Array||f.Uint8ClampedArray&&b instanceof Uint8ClampedArray?this._decodeRawImageUint8Array(b,a,c,d,e,h):Promise.reject(TypeError("'_decodeRawImage(source, width, height, stride, enumImagePixelFormat, templateName)': Type of 'source' should be 'Blob', 'ArrayBuffer' or 'Uint8Array'."))};
var k=a._handleRetJsonString=function(b){b="string"==typeof b||"object"==typeof b&&b instanceof String?JSON.parse(b):b;var d=a.EnumErrorCode;switch(b.exception){case d.DBR_SUCCESS:case d.DBR_LICENSE_INVALID:case d.DBR_LICENSE_EXPIRED:case d.DBR_1D_LICENSE_INVALID:case d.DBR_QR_LICENSE_INVALID:case d.DBR_PDF417_LICENSE_INVALID:case d.DBR_DATAMATRIX_LICENSE_INVALID:case d.DBR_DBRERR_AZTEC_LICENSE_INVALID:case d.DBR_RECOGNITION_TIMEOUT:if(b.textResult)for(d=0;d<b.textResult.length;++d){var e=b.textResult[d];
try{e.BarcodeText=c._bNodejs?Buffer.from(e.BarcodeText,"base64").toString():atob(e.BarcodeText)}catch(m){e.BarcodeText=""}}return b.textResult||b.localizationResultArray||b.Result||b.templateSettings||b.settings||b.outputSettings||b;default:throw a.BarcodeReaderException(b.exception,b.description);}};a.prototype.getAllLocalizationResults=function(){return this._localizationResultArray?this._localizationResultArray:k(this._instance.GetAllLocalizationResults())};a.prototype.getAllParameterTemplateNames=
function(){return k(this._instance.GetAllParameterTemplateNames())};a.prototype.getRuntimeSettings=function(){return this._instance?k(this._instance.GetRuntimeSettings()):this._runtimeSettings?JSON.parse(this._runtimeSettings):a._defaultRuntimeSettings};a.prototype.updateRuntimeSettings=function(b){var c=this;if("string"==typeof b||"object"==typeof b&&b instanceof String)var e=b;else if("object"==typeof b)e=JSON.stringify(b);else throw TypeError("'UpdateRuntimeSettings(settings)': Type of 'settings' should be 'String' or 'PlainObject'.");
if(c._instance)try{return k(c._instance.UpdateRuntimeSettings(e)),Promise.resolve()}catch(m){return Promise.reject(m)}else return Promise.resolve().then(function(){return c._instanceID?Promise.resolve():c._createWorkerInstance()}).then(function(){return new Promise(function(b,f){var h=Math.random();d[h]=function(a){return a.success?(c._runtimeSettings=e,b()):f(a.exception)};a._dbrWorker.postMessage({type:"updateRuntimeSettings",id:h,instanceID:c._instanceID,body:{settings:e}})})}).catch(function(b){return Promise.reject(b)})};
a.prototype.resetRuntimeSettings=function(){this._instance?k(this._instance.ResetRuntimeSettings()):this._runtimeSettings=null};a.prototype.outputSettingsToString=function(b){return k(this._instance.OutputSettingsToString(b||""))};a.prototype.initRuntimeSettingsWithString=function(b,a){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))if("object"==typeof b)b=JSON.stringify(b);else throw TypeError("'initRuntimeSettingstWithString(template, enumComflictMode)': Type of 'template' should be 'String' or 'PlainObject'.");
k(this._instance.InitRuntimeSettingstWithString(b,a?a:2))};a.prototype.appendTplStringToRuntimeSettings=function(b,a){if(!("string"==typeof b||"object"==typeof b&&b instanceof String))if("object"==typeof b)b=JSON.stringify(b);else throw TypeError("'appendTplStringToRuntimeSettings(template, enumComflictMode)': Type of 'template' should be 'String' or 'PlainObject'.");k(this._instance.AppendTplStringToRuntimeSettings(b,a?a:2))};a.EnumBarcodeFormat=a.EnumBarcodeFormat||{All:503317503,OneD:1023,CODE_39:1,
...
id:d.id,body:{success:!1,exception:b.message||b}})}})();break;case "updateRuntimeSettings":try{e[d.instanceID].updateRuntimeSettings(d.body.settings),postMessage({type:"task",id:d.id,body:{success:!0}})}catch(k){postMessage({type:"task",id:d.id,body:{success:!1,exception:k.message||k}})}break;case "resetRuntimeSettings":try{e[d.instanceID].resetRuntimeSettings(),postMessage({type:"task",id:d.id,body:{success:!0}})}catch(k){postMessage({type:"task",id:d.id,body:{success:!1,exception:k.message||k}})}break;
default:postMessage({type:"task",id:d.id,body:{success:!1,exception:"No such task."}})}}}();
dynamsoft.BarcodeReader.loadWasm=dynamsoft.BarcodeReader.loadWasm||function(){return new Promise(function(c,f){var a=dynamsoft.dbrEnv,e=a._self,d=dynamsoft.BarcodeReader;if("loaded"==d._loadWasmStatus)return c();d._loadWasmTaskQueue=d._loadWasmTaskQueue||function(){var a=new dynamsoft.TaskQueue;a.timeout=0;return a}();d._loadWasmTaskQueue.push(function(k){if("loaded"==d._loadWasmStatus)return d._loadWasmTaskQueue.next(),c();if(k)return d._loadWasmTaskQueue.next(),f(d._loadWasmStatus);d._loadWasmStatus=
...
kConsoleLog(a)};if("undefined"==typeof _dbrLoadWasm){void 0==d._bWithio&&(d._bWithio=a._bNodejs||a._bWorker&&(d._bCustomWorker||e.onmessage!=d._onWorkerMessage));var g=d._wasmjsName||"dbr-"+d._jsVersion+".wasm"+(d._bWithio?".withio":"")+".min.js";if(d._wasmjsResourcePath||d._resourcesPath){var k=d._wasmjsResourcePath||d._resourcesPath;"/"!=k.charAt(k.length-1)&&(k+="/");g=k+g}else a._bNodejs&&(g=require("path").join(__dirname,g));a._bWorker?importScripts(g):_dbrLoadWasm=require(g)}_dbrLoadWasm(f,
...
6,1,4,0,65,1,11]);try{var f=d.put(new WebAssembly.Module(e.buffer),"testStoreModule");f.onsuccess=function(){d.put(!0,"bSupportStoreModuleInDb").onsuccess=function(){b("set bSupportStoreModuleInDb = true success")}};f.onerror=function(){c("Failed to store [testStoreModule] in wasm cache, bSupportStoreModuleInDb == false: "+(f.error.message||f.error))}}catch(A){c("Failed to store [testStoreModule] in wasm cache, bSupportStoreModuleInDb == false: "+(A.message||A))}};d.onsuccess=function(){d.result?
b("bSupportStoreModuleInDb == true"):e()}})};d._lookupInDatabase=d._lookupInDatabase||function(a,b,c){return new Promise(function(d,e){var f=a.transaction([b]).objectStore(b).get(c);f.onsuccess=function(){f.result?d(f.result):e(c+" was not found in "+b)}})};d._storeInDatabase=d._storeInDatabase||function(a,b,c,d){return new Promise(function(e,f){var l=a.transaction([b],"readwrite").objectStore(b).put(d,c);l.onerror=function(){f("Failed to store "+c+" in wasm cache: "+(l.error.message||l.error))};
l.onsuccess=function(){e("Successfully stored "+c+" in "+b)}})};return d._openDb().then(function(a){e.kConsoleLog&&kConsoleLog("open db success");return k(a).then(function(a){e.kConsoleLog&&kConsoleLog(a);return Promise.resolve(!0)},function(a){e.kConsoleLog&&kConsoleLog(a.message||a);return Promise.resolve(!1)}).then(function(b){var l=(new Date).getTime();return d._lookupInDatabase(a,"wasm",h).then(function(c){if(c instanceof WebAssembly.Module){e.kConsoleLog&&kConsoleLog("get a wasm module from db, timecost:"+
((new Date).getTime()-l));var n=(new Date).getTime();return WebAssembly.instantiate(c,f).then(function(a){e.kConsoleLog&&kConsoleLog("build instance from module timecost: "+((new Date).getTime()-n));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve({module:c,instance:a})})}e.kConsoleLog&&kConsoleLog("get a wasm binary from db, timecost:"+((new Date).getTime()-l));var v=(new Date).getTime();return WebAssembly.instantiate(c,f).then(function(c){e.kConsoleLog&&
kConsoleLog("build instance from binary timecost: "+((new Date).getTime()-v));if(b){var f=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,c.module).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-f));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve(c)})}e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+((new Date).getTime()-g));return Promise.resolve(c)})},function(l){e.kConsoleLog&&
kConsoleLog(l.message||l);e.kConsoleLog&&kConsoleLog("downloading...");var n=(new Date).getTime();return b?WebAssembly.instantiateStreaming(fetch(c),f).then(function(b){d._onWasmDownloaded&&d._onWasmDownloaded();e.kConsoleLog&&kConsoleLog("download with build timecost: "+((new Date).getTime()-n));var c=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,b.module).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-c));e.kConsoleLog&&kConsoleLog("finish handle dbr wasm, total timecost: "+
((new Date).getTime()-g));return Promise.resolve(b)},function(a){e.kConsoleLog&&kConsoleLog(a.message||a);return Promise.reject("Can't store wasm in db.")})}):fetch(c).then(function(a){return a.arrayBuffer()}).then(function(b){e.kConsoleLog&&kConsoleLog("download timecost: "+((new Date).getTime()-n));d._onWasmDownloaded&&d._onWasmDownloaded();var c=(new Date).getTime();return d._storeInDatabase(a,"wasm",h,b).then(function(a){e.kConsoleLog&&kConsoleLog(a+", timecost: "+((new Date).getTime()-c));return Promise.resolve(b)},
...
dynamsoft.BarcodeReader.Scanner.prototype.open=function(){var c=this;if(c._isOpen)return Promise.reject("The scanner is already open.");c._isOpen=!0;var f=c.htmlElement,a,e,d,k,b;(function(){for(var c=[],n=$jscomp.makeIterator(f.children),g=n.next();!g.done;g=n.next())c.push(g.value);for(n=0;n<c.length;++n){var h=$jscomp.makeIterator(c[n].children);for(g=h.next();!g.done;g=h.next())c.push(g.value)}c=$jscomp.makeIterator(c);for(g=c.next();!g.done;g=c.next())g=g.value,g.classList.contains("dbrScanner-video")?
a=g:g.classList.contains("dbrScanner-sel-camera")?e=g:g.classList.contains("dbrScanner-sel-resolution")?(d=g,d.options.length||(d.innerHTML='<option class="dbrScanner-opt-gotResolution" value="got"></option><option data-width="3840" data-height="2160">ask 3840 x 2160</option><option data-width="2560" data-height="1440">ask 2560 x 1440</option><option data-width="1920" data-height="1080">ask 1920 x 1080</option><option data-width="1600" data-height="1200">ask 1600 x 1200</option><option data-width="1280" data-height="720">ask 1280 x 720</option><option data-width="800" data-height="600">ask 800 x 600</option><option data-width="640" data-height="480">ask 640 x 480</option><option data-width="640" data-height="360">ask 640 x 360</option>',
k=d.options[0])):g.classList.contains("dbrScanner-opt-gotResolution")?k=g:g.classList.contains("dbrScanner-btn-close")&&(b=g)})();var g=void 0,p=c._updateDevice=function(){return navigator.mediaDevices.enumerateDevices().then(function(b){var c=[],d;if(e){var f=e.value;e.innerHTML=""}for(var h=0;h<b.length;++h){var l=b[h];if("videoinput"==l.kind){var k={};k.deviceId=l.deviceId;k.label=l.label||"camera "+h;c.push(k)}}b=$jscomp.makeIterator(a.srcObject.getTracks());for(h=b.next();!h.done;h=b.next()){h=
h.value;if(m)break;if("video"==h.kind)for(l=$jscomp.makeIterator(c),k=l.next();!k.done;k=l.next())if(k=k.value,h.label==k.label){var m=k;g=k.deviceId;break}}if(e){b=$jscomp.makeIterator(c);for(k=b.next();!k.done;k=b.next())h=k.value,l=document.createElement("option"),l.value=h.deviceId,l.innerText=h.label,e.appendChild(l),f==h.deviceId&&(d=l);f=e.childNodes;if(!d&&m&&f.length)for(f=$jscomp.makeIterator(f),b=f.next();!b.done;b=f.next())if(b=b.value,m.label==b.innerText){d=b;break}d&&(e.value=d.value)}return Promise.resolve({current:m,
all:c})})};c._pausevideo=function(){a.pause()};var m=function(){a.srcObject&&(self.kConsoleLog&&self.kConsoleLog("======stop video========"),a.srcObject.getTracks().forEach(function(a){a.stop()}))},q=c._playvideo=function(b,e,f){return new Promise(function(h,l){m();self.kConsoleLog&&self.kConsoleLog("======before video========");var n=c.videoSettings=c.videoSettings||{video:{facingMode:{ideal:"environment"}}};/Safari/.test(navigator.userAgent)&&/iPhone/.test(navigator.userAgent)&&!/Chrome/.test(navigator.userAgent)?
1280<=e?n.video.width=1280:640<=e?n.video.width=640:320<=e&&(n.video.width=320):(e&&(n.video.width={ideal:e}),f&&(n.video.height={ideal:f}));b?(n.video.facingMode=void 0,n.video.deviceId={exact:b}):n.video.deviceId=n.video.deviceId||{ideal:g};var v=!1,z=function(){self.kConsoleLog&&self.kConsoleLog("======try getUserMedia========");self.kConsoleLog&&self.kConsoleLog("ask "+JSON.stringify(n.video.width)+"x"+JSON.stringify(n.video.height));navigator.mediaDevices.getUserMedia(n).then(function(b){self.kConsoleLog&&
self.kConsoleLog("======get video========");return new Promise(function(c,e){a.srcObject=b;a.onloadedmetadata=function(){self.kConsoleLog&&self.kConsoleLog("======play video========");a.play().then(function(){self.kConsoleLog&&self.kConsoleLog("======played video========");var b="got "+a.videoWidth+"x"+a.videoHeight;d&&k&&(k.setAttribute("data-width",a.videoWidth),k.setAttribute("data-height",a.videoHeight),d.value="got",k.innerText=b);self.kConsoleLog&&self.kConsoleLog(b);c();h({width:a.videoWidth,
height:a.videoHeight})},function(a){e(a)})};a.onerror=function(){e()}})}).catch(function(a){self.kConsoleLog&&self.kConsoleLog(a);!v&&n.video?(v=!0,n.video.deviceId=void 0,n.video.width=void 0,n.video.height=void 0,z()):l(a)})};z()})};c.arrDiffCodeInfo=[];var h,r=function(a,b){if(a instanceof Array){var c=[];a=$jscomp.makeIterator(a);for(var d=a.next();!d.done;d=a.next())c.push(r(d.value,b));return c}c=JSON.parse(JSON.stringify(a,function(a,b){if("oriVideoCanvas"!=a&&"searchRegionCanvas"!=a)return b}));
b||(c.oriVideoCanvas=a.oriVideoCanvas,c.searchRegionCanvas=a.searchRegionCanvas);return c},u=function(){if(c._isOpen){if(a.paused)return self.kConsoleLog&&self.kConsoleLog("Video is paused. Ask in 1s."),setTimeout(u,1E3);self.kConsoleLog&&self.kConsoleLog("======= once read =======");var b=(new Date).getTime();Promise.all([JSON.stringify(h.getRuntimeSettings())!=JSON.stringify(c.runtimeSettings)?h.updateRuntimeSettings(c.runtimeSettings):Promise.resolve(),function(){var b=c.searchRegion;if(b&&(b.sx||
b.sy||b.sWidth||b.sHeight||b.dWidth||b.dHeight)){var d=b.sx||0,e=b.sy||0,f=b.sWidth||a.videoWidth,g=b.sHeight||a.videoHeight,h=b.dWidth||b.sWidth||a.videoWidth;b=b.dHeight||b.sHeight||a.videoHeight;0<d&&1>d&&(d=Math.round(d*a.videoWidth));0<e&&1>e&&(e=Math.round(e*a.videoHeight));1>=f&&(f=Math.round(f*a.videoWidth));1>=g&&(g=Math.round(g*a.videoHeight));1>=h&&(h=Math.round(h*a.videoWidth));1>=b&&(b=Math.round(b*a.videoHeight));return[a,d,e,f,g,h,b]}return a}()]).then(function(a){a=a[1];a instanceof
Array||(a=[a]);h._bAddOriVideoCanvasToResult=c.bAddOriVideoCanvasToResult;h._bAddSearchRegionCanvasToResult=c.bAddSearchRegionCanvasToResult;return h.decodeVideo.apply(h,a)}).then(function(a){var d=(new Date).getTime();self.kConsoleLog&&self.kConsoleLog("time cost: "+(d-b)+"ms");for(var e=[],f=0;f<a.length;++f){var g=a[f],h=g.BarcodeText;self.kConsoleLog&&self.kConsoleLog(h);g.LocalizationResult.ExtendedResultArray[0].Confidence>=c.confidence&&e.push(g)}e=r(e);for(f=0;f<c.arrDiffCodeInfo.length;++f){g=
c.arrDiffCodeInfo[f];h=-1;for(var k in e){var l=e[k];if(g.result.BarcodeText==l.BarcodeText&&g.result.BarcodeFormat==l.BarcodeFormat){h=k;break}}-1!=h?(g.time=d,g.result=e[h],++g.count,e.splice(h,1)):d-g.time>c.duplicateForgetTime&&(c.arrDiffCodeInfo.splice(f,1),--f)}for(k=0;k<e.length;++k)for(f=e[k],g=k+1;g<e.length;)h=e[g],f.BarcodeText==h.BarcodeText&&f.BarcodeFormat==h.BarcodeFormat?e.splice(g,1):++g;f=$jscomp.makeIterator(e);for(k=f.next();!k.done;k=f.next())c.arrDiffCodeInfo.push({result:r(k.value,
!0),time:d,count:1});if(c.onFrameRead)c.onFrameRead(r(a));a=$jscomp.makeIterator(e);for(k=a.next();!k.done;k=a.next())if(d=k.value,c.onNewCodeRead)c.onNewCodeRead(d.BarcodeText,r(d));setTimeout(u,c.intervalTime)}).catch(function(a){self.kConsoleLog&&self.kConsoleLog(a);setTimeout(u,c.intervalTime);throw a;})}else h&&(h.deleteInstance(),h=void 0)},x=function(){q(e.value).then(function(){c._isOpen||m()}).catch(function(a){alert("Play video failed: "+(a.message||a))})};e&&e.addEventListener("change",
x);var w=function(){if(d&&-1!=d.selectedIndex){var a=d.options[d.selectedIndex];var b=a.getAttribute("data-width");a=a.getAttribute("data-height")}q(void 0,b,a).then(function(){c._isOpen||m()}).catch(function(a){alert("Play video failed: "+(a.message||a))})};d&&d.addEventListener("change",w);var t=!!f.parentNode,y=c._closeWindow=function(){m();c._isOpen=!1;e&&e.removeEventListener("change",x);d&&d.removeEventListener("change",w);c._closeWindow=void 0;b&&b.removeEventListener("click",y);var a=f.parentNode;
a&&!t&&a.removeChild(f)};b&&b.addEventListener("click",y);t||document.body.appendChild(f);return Promise.all([dynamsoft.BarcodeReader.loadWasm(),q()]).then(function(a){if(!c._isOpen)return m(),Promise.resolve();h=new dynamsoft.BarcodeReader;var b=h.getRuntimeSettings();b.mAntiDamageLevel=3;b.mDeblurLevel=0;for(var d in c.runtimeSettings)void 0!=b[d]&&(b[d]=c.runtimeSettings[d]);c.runtimeSettings=b;return Promise.all([a[1],h.updateRuntimeSettings(b)])}).then(function(a){u();return Promise.all([a[0],
p()])}).then(function(a){var b=a[0];a=a[1];return Promise.resolve({width:b.width,height:b.height,current:a.current,all:a.all})})};dynamsoft.BarcodeReader.Scanner.prototype.play=function(c,f,a){return this._isOpen?this._playvideo(c,f,a):Promise.reject("The scanner is not open.")};dynamsoft.BarcodeReader.Scanner.prototype.pause=function(){this._isOpen&&this._pausevideo()};dynamsoft.BarcodeReader.Scanner.prototype.close=function(){this._isOpen&&this._closeWindow()};
